...
def run_fixed_backtest(df, predictions, min_confidence=0.6):
    """수정된 백테스트 실행 - 올바른 수익 계산"""
    print("💰 수정된 백테스트 실행 중...")
    ...
    try:
            ...
            try:
                ...
                if signal['confidence'] >= min_confidence and signal['action'] != 'HOLD':
                    ...
                    if position != 0:
                        ...
                        trade_profit = exit_value - entry_capital
                        trade_profits.append(trade_profit)
                        
                        # 전략별 수익 기록
                        if current_strategy in strategy_profits:
                            strategy_profits[current_strategy].append(trade_profit)
                        
                        capital = exit_value
                        
                        # 청산 거래 기록
                        trades.append({
                            'type': 'SELL' if position == 1 else 'BUY',
                            'price': current_price,
                    ...
                    if signal['action'] == 'BUY':
                        ...
                        current_strategy = signal['strategy_used']
                        capital = 0
        ...
        if position != 0:
            ...
            final_trade_profit = exit_value - entry_capital
            trade_profits.append(final_trade_profit)
            
            if current_strategy in strategy_profits:
                strategy_profits[current_strategy].append(final_trade_profit)
...
def analyze_fixed_results(results):
    ...
    try:
        ...
        if trades:
            enter_trades = [t for t in trades if t.get('action') == 'ENTER']
            exit_trades = [t for t in trades if t.get('action') == 'EXIT']
            
            print(f"\n📊 거래 통계:")
            print(f"   총 진입: {len(enter_trades)}회")
            print(f"   총 청산: {len(exit_trades)}회")
            
            # 개별 거래 수익
            trade_profits = results.get('trade_profits', [])
            if trade_profits:
                ...
                print(f"   승률: {len(winning_trades)/len(trade_profits)*100:.1f}%")
                
                if winning_trades:
                    print(f"   평균 수익: {sum(winning_trades)/len(winning_trades):,.0f}원")
                if losing_trades:
                    print(f"   평균 손실: {sum(losing_trades)/len(losing_trades):,.0f}원")
            ...
            if strategy_stats:
                ...
                for strategy, stats in strategy_stats.items():
                    if stats['count'] > 0:
                        ...
                        print(f"         평균 수익: {stats['avg_profit']:,.0f}원")
...
def main():
    """메인 실행 함수"""
    try:
        parser = argparse.ArgumentParser(description='수정된 트리플 콤보 백테스트')
        ...
        # 4. 수정된 백테스트 실행
        results = run_fixed_backtest(df, predictions, args.min_confidence)
        
        # 5. 수정된 결과 분석
        analyze_fixed_results(results)
        
        print(f"\n⏰ 완료 시간: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("🎉 수정된 백테스트가 성공적으로 완료되었습니다!")
